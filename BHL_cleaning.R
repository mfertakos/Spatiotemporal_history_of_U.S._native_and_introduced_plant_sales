...
DB1<-BHL_RAG %>%
  dplyr::select(Source,ItemID,SearchedName,AcceptedName,USDAcode,Latitude,Longitude,City,State,PublicationDate,NurseryName,Title,ItemUrl)

DB2<-BHL_RAG %>%
  dplyr::select(USDAcode,AcceptedName, USDAstatus, invasive_status, Habit)
...
write.csv(DB1,'BHL_RAG_records.csv',row.names=FALSE,fileEncoding = "UTF-8")
write.csv(DB2,'BHL_RAG_taxa.csv',row.names=FALSE,fileEncoding = "UTF-8")
